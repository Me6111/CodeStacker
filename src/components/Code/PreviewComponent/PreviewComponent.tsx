...
import PropsEditor, { InputField } from "../PropsEditor/PropsEditor";
import CodeEditor from "../CodeEditor/CodeEditor";
import Dropdown, { DropdownItem } from "../../Dropdown/Dropdown/Dropdown";
import ResponsiveContainer, { ResponsiveStyle } from "../../ResponsiveContainer/ResponsiveContainer";
...
const STYLE_PROPS = ["size", "orientation", "proportion", "color", "backgroundColor", "borderColor", "hoverColor", "hoverBackgroundColor", "clickColor", "clickBackgroundColor", "focusColor", "activeColor"];

const UNIVERSAL_STYLE_OPTIONS = {
  size: ["xs", "sm", "md", "lg", "xl"],
  orientation: ["horizontal", "vertical"],
  proportion: ["even", "wide", "tall", "1:1", "1:2", "1:3", "1:4", "2:1", "3:1", "4:1"]
};
...
const PreviewComponent: React.FC<PreviewComponentProps> = ({ element }) => {
  ...
  const [meta, setMeta] = useState<{ options: Record<string, any[]>; numbers: string[]; colors: string[]; codeProps: string[] }>({ options: {}, numbers: [], colors: [], codeProps: [] });
  ...
  const [inheritedProps, setInheritedProps] = useState<Set<string>>(new Set());
  ...
  useEffect(() => {
    ...
    setCompReady(false);
    setPropsValues(null);
    setMeta({ options: {}, numbers: [], colors: [], codeProps: [] });
    setCompName("");
    setRenderKey(0);
    setPropWarnings([]);
    setInheritedProps(new Set());
    ...
    const tryLoad = () => {
      ...
      const colors: string[] = (mod[`${lower}ColorProps`] ?? []).map(String);
  ...
  const toggleInherited = (propName: string) => {
    setInheritedProps(prev => {
  ...
  const componentFields: InputField<any>[] = propsValues
    ? Object.entries(propsValues)
        .filter(([name]) => !STYLE_PROPS.includes(name))
        .map(([name, value]) => ({
          name,
          value,
          ...
          isColor: meta.colors.includes(name),
          isCode: meta.codeProps.includes(name),
        }))
    : [];

  const universalStyleFields: InputField<any>[] = [
    {
      name: "size",
      value: propsValues?.size ?? "md",
      setter: makeSetter("size"),
      options: UNIVERSAL_STYLE_OPTIONS.size,
      isInherited: inheritedProps.has("size"),
      toggleInherited: () => toggleInherited("size"),
      inheritedValue: undefined,
    },
    {
      name: "orientation",
      value: propsValues?.orientation ?? "horizontal",
      setter: makeSetter("orientation"),
      options: UNIVERSAL_STYLE_OPTIONS.orientation,
      isInherited: inheritedProps.has("orientation"),
      toggleInherited: () => toggleInherited("orientation"),
      inheritedValue: undefined,
    },
    {
      name: "proportion",
      value: propsValues?.proportion ?? "even",
      setter: makeSetter("proportion"),
      options: UNIVERSAL_STYLE_OPTIONS.proportion,
      isInherited: inheritedProps.has("proportion"),
      toggleInherited: () => toggleInherited("proportion"),
      inheritedValue: undefined,
    },
    {
      name: "color",
      value: propsValues?.color ?? "#ffffff",
      setter: makeSetter("color"),
      isColor: true,
      isInherited: inheritedProps.has("color"),
      toggleInherited: () => toggleInherited("color"),
      inheritedValue: undefined,
    },
    {
      name: "backgroundColor",
      value: propsValues?.backgroundColor ?? "#1a1a1a",
      setter: makeSetter("backgroundColor"),
      isColor: true,
      isInherited: inheritedProps.has("backgroundColor"),
      toggleInherited: () => toggleInherited("backgroundColor"),
      inheritedValue: undefined,
    },
    {
      name: "borderColor",
      value: propsValues?.borderColor ?? "#333333",
      setter: makeSetter("borderColor"),
      isColor: true,
      isInherited: inheritedProps.has("borderColor"),
      toggleInherited: () => toggleInherited("borderColor"),
      inheritedValue: undefined,
    },
    {
      name: "hoverColor",
      value: propsValues?.hoverColor ?? "#ffffff",
      setter: makeSetter("hoverColor"),
      isColor: true,
      isInherited: inheritedProps.has("hoverColor"),
      toggleInherited: () => toggleInherited("hoverColor"),
      inheritedValue: undefined,
    },
    {
      name: "hoverBackgroundColor",
      value: propsValues?.hoverBackgroundColor ?? "#2a2a2a",
      setter: makeSetter("hoverBackgroundColor"),
      isColor: true,
      isInherited: inheritedProps.has("hoverBackgroundColor"),
      toggleInherited: () => toggleInherited("hoverBackgroundColor"),
      inheritedValue: undefined,
    },
    {
      name: "clickColor",
      value: propsValues?.clickColor ?? "#ffffff",
      setter: makeSetter("clickColor"),
      isColor: true,
      isInherited: inheritedProps.has("clickColor"),
      toggleInherited: () => toggleInherited("clickColor"),
      inheritedValue: undefined,
    },
    {
      name: "clickBackgroundColor",
      value: propsValues?.clickBackgroundColor ?? "#333333",
      setter: makeSetter("clickBackgroundColor"),
      isColor: true,
      isInherited: inheritedProps.has("clickBackgroundColor"),
      toggleInherited: () => toggleInherited("clickBackgroundColor"),
      inheritedValue: undefined,
    },
    {
      name: "focusColor",
      value: propsValues?.focusColor ?? "#4caf50",
      setter: makeSetter("focusColor"),
      isColor: true,
      isInherited: inheritedProps.has("focusColor"),
      toggleInherited: () => toggleInherited("focusColor"),
      inheritedValue: undefined,
    },
    {
      name: "activeColor",
      value: propsValues?.activeColor ?? "#2196f3",
      setter: makeSetter("activeColor"),
      isColor: true,
      isInherited: inheritedProps.has("activeColor"),
      toggleInherited: () => toggleInherited("activeColor"),
      inheritedValue: undefined,
    },
  ];
  ...
  const editorContent = (
    <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", alignItems: "flex-start", justifyContent: "flex-start", padding: "20px 16px", boxSizing: "border-box", gap: "20px" }}>
      {showCode ? (
        ...
        <>
          <div style={{ width: "100%", display: "flex", justifyContent: "center" }}>
            <div style={{ width: "100%", maxWidth: "360px" }}>
              <div style={{ fontSize: "11px", fontWeight: 600, color: "#666", marginBottom: "12px", textTransform: "uppercase", letterSpacing: "0.05em" }}>Style Properties</div>
              <PropsEditor fields={universalStyleFields} />
            </div>
          </div>
          {componentFields.length > 0 && (
            <div style={{ width: "100%", display: "flex", justifyContent: "center" }}>
              <div style={{ width: "100%", maxWidth: "360px" }}>
                <div style={{ fontSize: "11px", fontWeight: 600, color: "#666", marginBottom: "12px", textTransform: "uppercase", letterSpacing: "0.05em" }}>Component Properties</div>
                <PropsEditor fields={componentFields} />
              </div>
            </div>
          )}
        </>
